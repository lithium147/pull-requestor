...
extractMultiline 'path' "$propertyExtractor" "$f.props" >> "$f.extracted"
...
replaceMultiline 'path' "$propertyReplacer" "$f"
...
if [ "$lineNum" == "" ]; then
  echo 'no properties block found, adding new block'
  lineNum=$(grep -n -E "(<parent>|<packaging>)" "$f" | head -n 1 | sed 's/:.*//')
  lineNum=$((lineNum - 1))
  # TODO add blank line between properties and dependencies
  echo '<properties>' > "$f.extractedWithPropertiesBlock"
  cat $f.extracted >> "$f.extractedWithPropertiesBlock"
  echo '</properties>' >> "$f.extractedWithPropertiesBlock"
  echo '' >> "$f.extractedWithPropertiesBlock"
  rm "$f.extracted"
  mv "$f.extractedWithPropertiesBlock" "$f.extracted"
fi

# insert the props back into original file
